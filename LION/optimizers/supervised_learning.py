...
class NETTSolver(LIONsolver):
    ...
    def validate(self):
        ...
        for index, (data, target) in tqdm(
            ...
            with torch.no_grad():
                output = self.model(data.to(self.device),target.to(self.device))
                validation_loss = self.validation_fn(output, target.to(self.device))
...
class ANETTSolver(LIONsolver):
    ...
    def mini_batch_step(self, data, target):
        ...
        self.optimizer.zero_grad()
        # Forward pass
        latent, output, noisy = self.model(data,target)
        # Compute loss
        self.loss = self.loss_fn(latent, output, target, self.model.q,noisy.to(latent.device))
        # Update optimizer and model
        self.loss.backward()
        self.optimizer.step()
        return self.loss.item()
